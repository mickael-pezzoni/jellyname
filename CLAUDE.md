...
- Validate the report before moving anything, and refuse to start on a bad report rather than warn-and-continue — but distinguish real corruption (missing `oldPath`/`newPath`) from a resolvable collision (duplicate `newPath` across items): only the former aborts the whole run, the latter just marks those specific items `"failed"` and lets everything else proceed.
- After a successful move, `apply` removes the source directory once it holds no video file left (not "once empty" — leftover posters/`.nfo` don't count, but an unmatched video does). This is bounded by `manifest.sourceRoot` (the original `--dir`, set by `scan`): that exact directory is never removed this way even if a root-level file leaving it makes it look empty, and reports written before this field existed (no `sourceRoot`) skip this cleanup entirely rather than guess. Confirmed by direct reproduction that omitting this bound deletes the scan root — treat this guard as load-bearing, not defensive-programming boilerplate.
...
`manifest.libraryRoot` (set from `scan`'s `--dest`, defaulting to `--dir`) is the root new paths are computed under — `resolve` needs it to build `newPath` for items that had none at scan time. `manifest.sourceRoot` (set from `scan`'s `--dir`) is different and is used only by `apply`'s source-cleanup safety guard, described below.
