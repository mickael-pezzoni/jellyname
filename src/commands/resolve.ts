...
function applyResolution(
  ...
  if (item.season === undefined || item.episode === undefined) {
    throw new Error(`item with no season/episode: ${item.oldPath}`);
  }
...
async function resolveUnmatchedItems(
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    console.log(`  reason: ${first.reason}${first.parsedTitle ? ` — detected title: "${first.parsedTitle}"` : ""}`);
    if (group.length > 1) console.log(`  ${group.length} episodes affected`);

    let query = await ask('  New search (title, "s" to skip, "q" to quit): ');
    ...
    while (candidates.length === 0) {
      ...
      query = await ask('  New search (or "s" to skip, "q" to quit): ');
      if (query === "q") {
        remaining.push(...group, ...rest);
        return { remaining, resolvedCount };
      }
    ...
    const choice = await ask('  Choice (number, "s" to skip, "q" to quit): ');
    ...
    if (!picked) {
      console.log("  Invalid choice, group left aside.");
      remaining.push(...group);
      continue;
    }
    ...
    for (const item of group) {
      ...
      if (type !== "movie" && (season === undefined || episode === undefined)) {
        const manual = await ask(`  Season and episode for ${basename(item.oldPath)} (e.g. 1x05), "s" to skip: `);
        const match = manual.match(/^(\d+)x(\d+)$/i);
        if (!match) {
          console.log("  Invalid format, item left aside.");
          remaining.push(item);
          continue;
        }
...
async function resolveAmbiguousGroup(
  ...
  while (true) {
    const answer = await ask('  Choice (number, new search, "s" to skip, "q" to quit): ');
...
export async function runResolve(argv: string[]): Promise<void> {
  ...
  if (typeof manifest.libraryRoot !== "string") {
    throw new Error(
      `${options.report} has no "libraryRoot" field — this report was generated by an older version of jellyname. Re-run "jellyname scan" to regenerate an up-to-date report before resolving ambiguous items.`,
    );
  }
  ...
  if (manifest.ambiguous.length > 0) {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      console.log(group.length === 1 ? `  ${basename(first.oldPath)}` : `  ${group.length} episodes affected`);
  ...
  console.log(
    `\n${totalResolved} resolved (${resolvedAmbiguousCount} ambiguous, ${resolvedUnmatchedCount} unmatched), ${skippedAmbiguousCount} skipped, ${remainingAmbiguous.length} ambiguous remaining, ${remainingUnmatched.length} unmatched remaining.`,
  );
}
